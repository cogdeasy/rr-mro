...
function RequestDetail() {
  ...
  const loadRequest = (reqId: string) => {
    api.getRequest(reqId).then(r => {
      setRequest(r);
  ...
  const handleUpdateStatus = (status: string) => {
    if (!request) return;
    api.updateStatus(request.id, status, 'Dr. J. Richardson').then(r => {
      setRequest(r);
      setAvailableActions(STATUS_FLOW[r.status] || []);
    });
  };
  ...
  const runTriage = () => {
    if (!request) return;
    api.triageRequest(request.id).then(() => loadRequest(request.id));
  };

  const generateDoc = () => {
    if (!request) return;
    api.generateDocument(request.id, 'Dr. J. Richardson').then(() => loadRequest(request.id));
  };

  const handleAddComment = () => {
    if (!request || !newComment.trim()) return;
    api.addComment(request.id, {
      content: newComment,
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '2rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          {!request.triageResult && (
            <button className="btn-navy" onClick={runTriage} style={{ fontSize: '0.8125rem', padding: '0.5rem 1rem' }}>
              Run AI Triage
            </button>
          )}
          {!request.generatedDocument && (
            <button className="btn-navy" onClick={generateDoc} style={{ fontSize: '0.8125rem', padding: '0.5rem 1rem' }}>
              Generate Document
            </button>
          )}
        </div>
