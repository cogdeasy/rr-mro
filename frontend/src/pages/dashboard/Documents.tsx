...
import { VarianceRequestSummary, STATUS_CSS, STATUS_LABELS } from '../../models/variance-request';
          ...
          <tbody>
            {documentedRequests.map(r => (
              <tr key={r.id} style={{ borderTop: '1px solid var(--rr-border)' }}>
                ...
                <td className="td"><span className={`badge ${STATUS_CSS[r.status] || r.status.toLowerCase()}`}>{STATUS_LABELS[r.status] || r.status}</span></td>
                <td className="td">
