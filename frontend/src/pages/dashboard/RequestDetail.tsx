...
import { getRequest, updateStatus, triageRequest, generateDocument, addComment } from '../../services/api';
import { VarianceRequest, STATUS_LABELS, STATUS_CSS } from '../../models/variance-request';

const STATUS_FLOW: Record<string, { status: string; label: string }[]> = {
...
export default function RequestDetail() {
  ...
  function loadRequest(requestId: string) {
    getRequest(requestId).then(r => {
      setRequest(r);
      setAvailableActions(STATUS_FLOW[r.status] || []);
    });
  }
  ...
  function handleUpdateStatus(status: string) {
    if (!request) return;
    updateStatus(request.id, status, 'Dr. J. Richardson').then(r => {
      setRequest(r);
      setAvailableActions(STATUS_FLOW[r.status] || []);
    });
  }
  ...
  function handleRunTriage() {
    if (!request) return;
    triageRequest(request.id).then(() => loadRequest(request.id));
  }

  function handleGenerateDoc() {
    if (!request) return;
    generateDocument(request.id, 'Dr. J. Richardson').then(() => loadRequest(request.id));
  }
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '2rem' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem' }}>
            ...
            <span className={`badge ${STATUS_CSS[request.status] || ''}`}>{STATUS_LABELS[request.status] || request.status}</span>
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          {!request.triageResult && (
            <button className="btn-navy" onClick={handleRunTriage} style={{ fontSize: '0.8125rem', padding: '0.5rem 1rem' }}>Run AI Triage</button>
          )}
          {!request.generatedDocument && (
            <button className="btn-navy" onClick={handleGenerateDoc} style={{ fontSize: '0.8125rem', padding: '0.5rem 1rem' }}>Generate Document</button>
          )}
        </div>
      </div>
            ...
            <div className="card" style={{ borderLeft: '3px solid var(--rr-gold)' }}>
              ...
              <div style={{ display: 'grid', gap: '1rem' }}>
                {request.generatedDocument.problemStatement && (
                  <div>
                    <p style={{ fontSize: '0.75rem', color: 'var(--rr-muted)', marginBottom: '0.25rem', fontWeight: 600 }}>Problem Statement</p>
                    <p style={{ fontSize: '0.8125rem', lineHeight: 1.6 }}>{request.generatedDocument.problemStatement}</p>
                  </div>
                )}
                {request.generatedDocument.technicalAnalysis && (
                  <div>
                    <p style={{ fontSize: '0.75rem', color: 'var(--rr-muted)', marginBottom: '0.25rem', fontWeight: 600 }}>Technical Analysis</p>
                    <p style={{ fontSize: '0.8125rem', lineHeight: 1.6 }}>{request.generatedDocument.technicalAnalysis}</p>
                  </div>
                )}
                {request.generatedDocument.safetyAssessment && (
                  <div>
                    <p style={{ fontSize: '0.75rem', color: 'var(--rr-muted)', marginBottom: '0.25rem', fontWeight: 600 }}>Safety Assessment</p>
                    <p style={{ fontSize: '0.8125rem', lineHeight: 1.6 }}>{request.generatedDocument.safetyAssessment}</p>
                  </div>
                )}
                {request.generatedDocument.proposedDisposition && (
                  <div>
                    <p style={{ fontSize: '0.75rem', color: 'var(--rr-muted)', marginBottom: '0.25rem', fontWeight: 600 }}>Proposed Disposition</p>
                    <p style={{ fontSize: '0.8125rem', lineHeight: 1.6 }}>{request.generatedDocument.proposedDisposition}</p>
                  </div>
                )}
                {request.generatedDocument.regulatoryReferences && (
                  <div>
                    <p style={{ fontSize: '0.75rem', color: 'var(--rr-muted)', marginBottom: '0.25rem', fontWeight: 600 }}>Regulatory References</p>
                    <p style={{ fontSize: '0.8125rem', lineHeight: 1.6 }}>{request.generatedDocument.regulatoryReferences}</p>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="card">
            ...
            {request.auditTrail.map(a => (
              <div key={a.id} style={{ display: 'flex', gap: '0.75rem', padding: '0.5rem 0', borderBottom: '1px solid var(--rr-border)' }}>
                <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: 'var(--rr-navy)', marginTop: '6px', flexShrink: 0 }}></div>
                <div style={{ flex: 1 }}>
          ...
          <div className="card">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {availableActions.map(action => (
                <button key={action.status} className="btn-outline" style={{ justifyContent: 'center', fontSize: '0.8125rem' }}
                  onClick={() => handleUpdateStatus(action.status)}>{action.label}</button>
              ))}
            </div>
          </div>
