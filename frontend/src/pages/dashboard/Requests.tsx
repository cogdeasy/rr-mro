...
import { getRequests, getEngineTypes } from '../../services/api';
...
export default function Requests() {
  ...
  useEffect(() => { getEngineTypes().then(setEngineTypes); }, []);
  useEffect(() => { loadRequests(); }, [page, statusFilter, priorityFilter, engineFilter, search]);
    ...
    <div style={{ padding: '2rem' }}>
      ...
      <div className="card" style={{ marginBottom: '1rem', padding: '1rem' }}>
        <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
          <input type="text" value={search} onChange={e => { setSearch(e.target.value); onFilter(); }}
          ...
          <tbody>
            {requests.map(r => (
              <tr key={r.id} className="table-row" onClick={() => navigate(`/dashboard/requests/${r.id}`)} style={{ cursor: 'pointer' }}>
                ...
                <td className="td muted">{new Date(r.createdAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {totalCount > pageSize && (
        <div style={{ display: 'flex', justifyContent: 'center', gap: '0.5rem', marginTop: '1rem' }}>
          <button className="btn-outline" onClick={() => setPage(p => p - 1)} disabled={page <= 1} style={{ padding: '0.375rem 0.75rem', fontSize: '0.8125rem' }}>Prev</button>
          <span style={{ padding: '0.375rem 0.75rem', fontSize: '0.8125rem', color: 'var(--rr-muted)' }}>{page} / {totalPages}</span>
          <button className="btn-outline" onClick={() => setPage(p => p + 1)} disabled={page >= totalPages} style={{ padding: '0.375rem 0.75rem', fontSize: '0.8125rem' }}>Next</button>
        </div>
      )}
    </div>
  );
