...
import { VarianceRequestSummary, STATUS_LABELS, STATUS_CSS } from '../models/variance-request';
...
export default function Track() {
  ...
  useEffect(() => { loadRequests(); }, [page, statusFilter, priorityFilter, search]);
        ...
        <div className="container">
              ...
              <tbody>
                {requests.map(r => (
                  <tr key={r.id}
                    ...
                    <td style={{ padding: '0.75rem 1rem' }}><span className={`badge ${STATUS_CSS[r.status] || ''}`}>{STATUS_LABELS[r.status] || r.status}</span></td>
          ...
          {totalCount > pageSize && (
            <div style={{ display: 'flex', justifyContent: 'center', gap: '0.5rem', marginTop: '1.5rem' }}>
              <button className="btn-outline" onClick={() => setPage(p => p - 1)} disabled={page <= 1}>Previous</button>
              <span style={{ padding: '0.625rem 1rem', fontSize: '0.875rem', color: 'var(--rr-muted)' }}>Page {page} of {totalPages}</span>
              <button className="btn-outline" onClick={() => setPage(p => p + 1)} disabled={page >= totalPages}>Next</button>
            </div>
          )}
        </div>
      </section>
