      ...
      <div style={{ display: 'grid', gap: '1.5rem', maxWidth: 800 }}>
        <div className="card">
          ...
          <div style={{ display: 'grid', gap: '1rem' }}>
            {[
              { name: 'Initiate Agent', desc: 'Front-load RFI questions on submission' },
              { name: 'Scoping / Triage Agent', desc: 'Severity classification and specialist routing' },
              { name: 'Document Authoring Agent', desc: 'RAG-powered variance document generation' },
            ].map((agent) => (
              <div key={agent.name} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem', background: 'var(--rr-platinum)', borderRadius: '0.375rem' }}>
                <div>
                  <p style={{ fontSize: '0.8125rem', fontWeight: 500 }}>{agent.name}</p>
                  <p style={{ fontSize: '0.75rem', color: 'var(--rr-muted)' }}>{agent.desc}</p>
                </div>
                <span style={{ fontSize: '0.75rem', fontWeight: 500, color: '#16A34A', background: '#F0FDF4', padding: '0.25rem 0.5rem', borderRadius: 4 }}>Active</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div style={{ display: 'grid', gap: '0.75rem' }}>
            {regulations.map((reg) => (
              <div key={reg.name} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.5rem 0', borderBottom: '1px solid var(--rr-border)' }}>
                <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--rr-navy)' }} />
                <div>
