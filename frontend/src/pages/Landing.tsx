    ...
    <>
      <section className="gradient-hero" style={{ minHeight: '80vh', display: 'flex', alignItems: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="container" style={{ position: 'relative', zIndex: 1, width: '100%' }}>
          <div style={{ maxWidth: 720 }}>
            ...
            <p style={{ fontSize: '1.125rem', color: 'rgba(255,255,255,0.7)', lineHeight: 1.6, marginBottom: '2.5rem', maxWidth: 600 }}>
              Digitising the technical variance process for the Rolls-Royce MRO network. AI-assisted triage, document authoring, and end-to-end workflow management across all engine programmes.
            </p>
            <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
              <Link to="/dashboard/submit" className="btn-primary" style={{ fontSize: 15, padding: '0.875rem 2rem' }}>Submit Variance Request</Link>
              <Link to="/dashboard/track" className="btn-secondary" style={{ fontSize: 15, padding: '0.875rem 2rem' }}>Track Requests</Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: '5rem 0', background: 'var(--rr-platinum)' }}>
        <div className="container">
          <div className="section-label"><div className="line" /><span>Platform Capabilities</span></div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem' }}>
            {[
              ...
              <div key={cap.title} className="card">
                <div style={{ width: 40, height: 40, borderRadius: 8, background: 'var(--rr-navy)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1rem' }}>
                  <span style={{ fontSize: 20 }}>{cap.icon}</span>
                </div>
                <h3 style={{ fontSize: '1.125rem', fontWeight: 600, marginBottom: '0.5rem' }}>{cap.title}</h3>
                <p style={{ fontSize: '0.875rem', color: 'var(--rr-muted)', lineHeight: 1.6 }}>{cap.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: '5rem 0' }}>
        <div className="container">
          <div className="section-label"><div className="line" /><span>Workflow</span></div>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '2rem', flexWrap: 'wrap' }}>
            {engines.map((engine) => (
              <div key={engine} className="engine-card">
                <span style={{ fontSize: 15, fontWeight: 600, color: '#fff' }}>{engine}</span>
              </div>
            ))}
          </div>
        </div>
