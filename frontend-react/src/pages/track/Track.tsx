...
import { getRequests } from '../../shared/services/api';
...
export default function Track() {
  ...
  function loadRequests(p = page, s = search, st = statusFilter, pr = priorityFilter) {
    getRequests({
      page: p, pageSize,
      status: st || undefined,
  ...
  function onFilter(newSearch?: string, newStatus?: string, newPriority?: string) {
    const s = newSearch ?? search;
    const st = newStatus ?? statusFilter;
    const pr = newPriority ?? priorityFilter;
    setPage(1);
    loadRequests(1, s, st, pr);
  }
          ...
          <div className="card" style={{ marginBottom: '1.5rem' }}>
            <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'end' }}>
              <div className="form-group" style={{ marginBottom: 0, flex: 1, minWidth: '200px' }}>
                <label>Search</label>
                <input type="text" value={search} onChange={e => { setSearch(e.target.value); onFilter(e.target.value); }} placeholder="Search by reference, title, or description" />
              </div>
