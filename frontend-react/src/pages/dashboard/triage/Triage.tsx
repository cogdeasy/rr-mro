      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
        <div className="card">
          ...
          {awaitingTriage.length === 0 && (
            <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--rr-muted)', fontSize: '0.875rem' }}>
        ...
        <div className="card">
          ...
          {triaged.map(r => (
            <div key={r.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.75rem 0', borderBottom: '1px solid var(--rr-border)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <span className={`badge ${STATUS_CSS[r.status] || ''}`}>{STATUS_LABELS[r.status] || r.status}</span>
