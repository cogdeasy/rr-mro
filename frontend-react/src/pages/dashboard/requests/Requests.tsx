...
import { getRequests, getEngineTypes } from '../../../shared/services/api';
import { STATUS_LABELS, STATUS_CSS } from '../../../shared/models/variance-request.model';
import type { VarianceRequestSummary } from '../../../shared/models/variance-request.model';
import './Requests.css';
...
export default function Requests() {
  ...
  function loadRequests(p = page, s = search, st = statusFilter, pr = priorityFilter, eng = engineFilter) {
    getRequests({
      page: p, pageSize,
      status: st || undefined,
  ...
  function onFilter(newSearch?: string, newStatus?: string, newPriority?: string, newEngine?: string) {
    const s = newSearch ?? search;
    const st = newStatus ?? statusFilter;
    const pr = newPriority ?? priorityFilter;
    const eng = newEngine ?? engineFilter;
    setPage(1);
    loadRequests(1, s, st, pr, eng);
  }
    ...
    <div style={{ padding: '2rem' }}>
      ...
      <div className="card" style={{ marginBottom: '1rem', padding: '1rem' }}>
        <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
          <input type="text" value={search} onChange={e => { setSearch(e.target.value); onFilter(e.target.value); }} placeholder="Search requests..."
          ...
          <select value={statusFilter} onChange={e => { setStatusFilter(e.target.value); onFilter(undefined, e.target.value); }}
            style={{ padding: '0.5rem 0.75rem', fontSize: '0.8125rem', border: '1px solid var(--rr-border)', borderRadius: '0.375rem' }}>
            <option value="">All Statuses</option>
            {statusOptions.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}
          </select>
          <select value={priorityFilter} onChange={e => { setPriorityFilter(e.target.value); onFilter(undefined, undefined, e.target.value); }}
            style={{ padding: '0.5rem 0.75rem', fontSize: '0.8125rem', border: '1px solid var(--rr-border)', borderRadius: '0.375rem' }}>
          ...
          <select value={engineFilter} onChange={e => { setEngineFilter(e.target.value); onFilter(undefined, undefined, undefined, e.target.value); }}
            style={{ padding: '0.5rem 0.75rem', fontSize: '0.8125rem', border: '1px solid var(--rr-border)', borderRadius: '0.375rem' }}>
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {requests.map(r => (
              <tr key={r.id} className="table-row" onClick={() => navigate(`/dashboard/requests/${r.id}`)}>
                ...
                <td className="td"><span className={`badge ${STATUS_CSS[r.status] || ''}`}>{STATUS_LABELS[r.status] || r.status}</span></td>
