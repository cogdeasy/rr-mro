          ...
          <tbody>
            {documentedRequests.map(r => (
              <tr key={r.id} style={{ borderTop: '1px solid var(--rr-border)' }}>
                ...
                <td style={{ padding: '0.625rem 0.75rem', fontSize: '0.8125rem' }}><span className={`badge ${STATUS_CSS[r.status] || r.status.toLowerCase()}`}>{STATUS_LABELS[r.status] || r.status}</span></td>
